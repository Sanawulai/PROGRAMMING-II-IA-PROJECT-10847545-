...
namespace InventorySystemCsharp
{
    public partial class Admin_home : Form
    {
        ...
        public string con = "datasource=localhost:127.0.0.1;port=3306;username='roor';password=@sanawulai123;database=inventorymgcsharp";
        public Admin_home()
        {
        ...
        private void add_manager_btn_Click(object sender, EventArgs e)
        {
            ...
            using(conn=new MySqlConnection(con))
            {
                ...
                MySqlDataReader newt=cmd.ExecuteReader();
                if (newt.HasRows)
                {
                    while (newt.Read())
                    {
                        Console.WriteLine(newt.GetValue(1));
                    }
                }
            ...
            if (MfnameTxt.Text != "" && MlnameTxt.Text != "" && MusernameTxt.Text != "" && MphonenumTxt.Text != "" && MpassTxt.Text != "" && MrepassTxt.Text != "" && typecomboTxt.Text != "")
            {
                if (MpassTxt.Text == MrepassTxt.Text)
                {
                    try
                    {
                        ...
                        MySqlConnection conn = new MySqlConnection(@"datasource=127.0.0.1;port=3306;SslMode=none;username=root;password=maxwelloppongmarfo1@;database=inventorymgcsharp;");
